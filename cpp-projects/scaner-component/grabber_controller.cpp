...
GrabberController::GrabberController(UdpReaderManager *udpReaderManager, std::vector<Interface> *localInterfaces, size_t id, K2GrabberTargetInfo info) : idC(id), m_info(info){

    // init workers
    m_udpReaderManager  = udpReaderManager;
    m_tcpSender         = std::make_unique<ScanerTcpSender>(id, localInterfaces);
    m_dataProcess       = std::make_unique<ProcessData>(id);
...
void GrabberController::init_connections(){
    ...
    m_udpReaderManager->all_packets_received_signal.connect([&](size_t idInterface, size_t idGrabber, std::vector<std::vector<char>*> *packets){
        ...
        if(idInterface == m_tcpSender->reading_interface() &&idGrabber == idC){
            ...
            m_frameReaderT = std::make_unique<std::thread>(&FrameReader::process_packets, m_frameReader.get(), packets);
        }
    });    
...
//        message_signal("Timeout from interface " + std::to_string(interface));
    ...
    m_frameReader->frame_received_signal.connect([&](K2UdpHeader header, std::shared_ptr<camera::K2Frame> frame){
        ...
        m_dataProcessT = std::make_unique<std::thread>(&ProcessData::process_data_from_udp_frame, m_dataProcess.get(), header, frame);
    });
...
bool GrabberController::connect_grabber(){
...
void GrabberController::set_camera_parameters(K2Settings parameters){
