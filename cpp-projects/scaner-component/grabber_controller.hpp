...
// local
#include "network/scaner_tcp_sender.hpp"
#include "network/udp_reader_manager.hpp"
#include "network/frame_reader.hpp"
...
public :
    ...
    GrabberController(network::UdpReaderManager *udpReaderManager, std::vector<network::Interface> *localInterfaces, size_t id, tool::camera::K2GrabberTargetInfo info);
...
private:
    ...
    // tcp sender
    network::TcpSenderUP m_tcpSender = nullptr;

    // udp reader
    network::UdpReaderManager *m_udpReaderManager = nullptr;

    // data processing
    ProcessDataUP m_dataProcess = nullptr;
    std::unique_ptr<std::thread> m_dataProcessT = nullptr;

    // frame reader
    network::FrameReaderUP m_frameReader = nullptr;
