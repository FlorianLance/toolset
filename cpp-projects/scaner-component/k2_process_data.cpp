...
K2ProcessData::K2ProcessData(size_t id) : m_p(std::make_unique<Impl>(id)){
}

K2ProcessData::~K2ProcessData(){
    tjDestroy(m_p->jpegDecompressor);
}

K2CloudDisplayData *K2ProcessData::cloud_data(){
    return &m_p->cloudData;
}

K2MeshDisplayData *K2ProcessData::mesh_data(){
    return &m_p->meshData;
}

auto K2ProcessData::process_data_from_udp_frame(K2UdpHeader header, std::shared_ptr<K2Frame> frame) -> void {
...
auto K2ProcessData::process_compressed_color_1920x1080() -> void {
...
auto K2ProcessData::process_compressed_color_512x424() -> void {
...
auto K2ProcessData::process_depth() -> void {
...
auto K2ProcessData::process_infra() -> void {
...
auto K2ProcessData::process_compressed_color_cloud() -> void {
