...
struct UdpReaderManager::Impl{
    std::vector<ScanerUdpReaderUP> readers;
};

UdpReaderManager::UdpReaderManager(const std::vector<Interface> &interfaces) : m_p(std::make_unique<Impl>()){

    // init interface readers and threads
    for(size_t ii = 0; ii < interfaces.size(); ++ii){
        m_p->readers.emplace_back(std::make_unique<ScanerUdpReader>(ii, interfaces[ii]));
    }
