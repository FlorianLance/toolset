...
ScanerUdpReader::ScanerUdpReader(size_t id, Interface interface) : m_p(std::make_unique<Impl>()){
...
ScanerUdpReader::~ScanerUdpReader(){
    disable_reading();
}

size_t ScanerUdpReader::id() const{
    return m_p->idInterface;
}

Interface ScanerUdpReader::interface() const{
    return m_p->interface;
}

bool ScanerUdpReader::enable_reading(int readingPort){
    ...
    if(m_p->connectionValid){
        ...
        m_p->thread = std::thread(&ScanerUdpReader::read_data, this);
...
void ScanerUdpReader::disable_reading(){
...
void ScanerUdpReader::clean_socket(){
...
void ScanerUdpReader::read_data(){
