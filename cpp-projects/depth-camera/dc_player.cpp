    ...
    auto uncompress_frame(size_t idCamera) -> bool{
        if(idCamera < camerasFrameId.size()){
            if(camerasCompressedFrame[idCamera] != nullptr){
                ...
                std::cout << "id " << camerasCompressedFrame[idCamera]->idCapture << " " << camerasFrame[idCamera]->idCapture << "\n";
...
auto DCPlayer::start_playing() noexcept -> void{
    i->data.start_playing();
    update_states();
}

auto DCPlayer::stop_playing() noexcept -> void{
    i->data.stop_playing();
    update_states();
}
...
auto DCPlayer::restart() noexcept -> void{
    i->data.set_current_time(0.0);
    update_states();
}

auto DCPlayer::set_current_time(double timeMs) noexcept -> void{
    i->data.set_current_time(timeMs);
    update_states();
}

auto DCPlayer::update() -> void{
...
auto DCPlayer::is_playing() const noexcept -> bool{
...
auto DCPlayer::update_states() noexcept -> void{
